    ...
    products.forEach((prod) =>{
         const {sys:{id},productName,unit,category,avilability,image,price,benefitsAndValues1,benefitsAndValues2,benefitsAndValues3,benefitsAndValues4,benefitsAndValues5} = prod
          ...
          if(div.querySelector("button")){
            ...
            isUserLoggedIn(user =>{
              if(user){
                div.querySelector("button").removeEventListener("click",addProdHandler)
                console.log("add to DB :)");

              }else{
                // if no user Or user logged out use localstorage to save his product

                // avoid dublicating listener when user switch log in and out 
                div.querySelector("button").removeEventListener("click",addProdHandler)
                div.querySelector("button").addEventListener("click",addProdHandler)
              }
            })
